...
export class StaffController {
  ...
  static getMe = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const staff = await StaffService.getByProfileId(req.user!.id);
      res.json({ success: true, data: staff });
    },
  );

  static listByDepartment = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
  ...
  static changeRole = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
  ...
  static deletePermanent = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
  ...
  static export = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      const exportData = await StaffService.export(
      ...
      if (format === "csv") {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename=staff_export_${Date.now()}.csv`,
        );
        res.send(exportData);
      } else {
