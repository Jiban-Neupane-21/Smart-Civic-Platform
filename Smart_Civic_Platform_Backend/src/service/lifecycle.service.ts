...
export class LifecycleService {
  ...
  async transition(
    ...
    // Resolve author profile ID if staff.id was passed
    let authorProfileId = actorId;
    if (actorId && actorRole === "staff") {
      const { data: staffRec } = await this.supabaseAdmin
        .from("staff")
        .select("profile_id")
        .eq("id", actorId)
        .maybeSingle();
      if (staffRec?.profile_id) {
        authorProfileId = staffRec.profile_id;
      }
    }
    ...
    await this.supabaseAdmin.from("complaint_updates").insert({
      ...
      note: note || this.getCitizenFriendlyMessage(targetStatus, note),
      is_internal: false,
    });
  ...
  async getTimeline(complaintId: string) {
    const { data, error } = await this.supabaseAdmin
      ...
      .select(`
        id, note, is_internal, created_at, author_id,
    ...
    return (data || []).map((item: any) => ({
      ...
      updated_by_name: item.author?.full_name || "System",
      updated_by_role: item.author?.role || "system",
    }));
  }
}
