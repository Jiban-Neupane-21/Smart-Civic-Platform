...
export const validateMunicipalityRegister = [
  body('official_name').trim().notEmpty().withMessage('Official name is required'),
  body('login_email').isEmail().normalizeEmail().withMessage('Valid login email required'),
  body('head_full_name').trim().notEmpty().withMessage('Head full name is required'),
...
export const validateDepartmentRegister = [
  ...
  body('head_full_name').trim().notEmpty().withMessage('Head full name is required'),
  body('head_email').isEmail().normalizeEmail().withMessage('Valid head email required')
//   strongPassword.builder.withMessage('head_password must be strong') 
    || body('head_password').isLength({ min: 8 }).withMessage('Password min 8 chars')
];
...
export const validateCitizenRegister = [
  ...
  body('email').isEmail().normalizeEmail().withMessage('Valid email required'),
  body('municipality_id').optional().isUUID().withMessage('municipality_id must be a UUID'),
  strongPassword
];
